...
mod tests {
    ...
    fn default_bundles_are_selected_by_default() {
        ...
        assert!(selected.contains(&pkg_key(1, "EchoClient 1.21.11 Fabric [QoL]", "qol-a")));
        assert!(selected.contains(&pkg_key(1, "EchoClient 1.21.11 Fabric [Utility]", "util-a")));
    }
    ...
    fn files_the_bundle_ships_off_are_not_selected_by_default() {
        ...
        assert!(!selected.contains(&pkg_key(
            1,
            "EchoClient 1.21.11 Fabric [Utility]",
            "util-opt"
        )));
    }
    ...
    fn migrated_categories_opt_into_their_bundles_without_losing_defaults() {
        ...
        assert!(selected.contains(&pkg_key(1, "EchoClient 1.21.11 Fabric [SkyBlock]", "sb-a")));
        assert!(selected.contains(&pkg_key(1, "EchoClient 1.21.11 Fabric [QoL]", "qol-a")));
        assert!(!selected.iter().any(|k| k.contains("[PvP]")));
    }
    ...
    fn hidden_files_are_never_selected() {
        ...
        assert!(selected.contains(&pkg_key(1, "EchoClient 1.21.11 Fabric [QoL]", "shown")));
        assert!(!selected.contains(&pkg_key(1, "EchoClient 1.21.11 Fabric [QoL]", "dep")));
    }
